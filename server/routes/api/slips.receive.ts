...
export default defineHandler(async (event) => {
  ...
  if (slip.status !== "pr_open" && slip.status !== "partial") return new Response(JSON.stringify({ error: "Only open PRs can be marked received" }), { status: 400, headers: { "content-type": "application/json" } });
  const inbound = (slip.qty as number) - (slip.issued_qty as number);
  if (inbound <= 0) return new Response(JSON.stringify({ error: "Nothing left to receive" }), { status: 400, headers: { "content-type": "application/json" } });
