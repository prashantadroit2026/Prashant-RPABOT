...
test("the auth schema ships outside the globbed directory", () => {
  ...
  const topFiles = readdirSync(migrationsDir);
  // Auth schema must stay in migrations/auth/ until sign-in is turned on —
  // it must not appear as a pending migration at the top level.
  const pending = pendingMigrations(topFiles, []);
  assert.ok(
    !pending.some((p) => p.name === AUTH_MIGRATION),
    `auth schema ${AUTH_MIGRATION} should not be at top level when auth is off; got pending ${JSON.stringify(pending.map((p) => p.name))}`,
  );
  // App migrations (like 0002_plant.sql) are expected at top level and
  // should be the only pending migrations when no migrations have been applied.
  for (const p of pending) {
    assert.ok(
      p.name !== AUTH_MIGRATION,
      `${p.name} should not be the auth schema`,
    );
  }
