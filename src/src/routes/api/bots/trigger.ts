  ...
  server: {
    handlers: {
      POST: async ({ request }) => {
        ...
        if ((bot as Record<string, unknown>).status === "paused") return Response.json({ error: "Bot is paused — resume before triggering" }, { status: 400 });
        ...
        if (isRpa) {
          ...
          return Response.json(run as unknown as Record<string, unknown>);
        }
        const started = new Date();
